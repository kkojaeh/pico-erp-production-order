...
@SpringBootTestComponent(parent = TestParentApplication, siblings = [
  UserApplication, ItemApplication, ProjectApplication, ProcessApplication,
  CompanyApplication, WarehouseApplication
])
@Transactional
@Rollback
@ActiveProfiles("test")
class MessageSourceSpec extends Specification {
  ...
  def "구매요청 상태"() {
    when:
    def messages = Stream.of(ProductionOrderStatusKind.values())
      .map({
